...
@param {int} n The string length (i.e. the number of octets).
@return {string} A hex string (padded) to size N octets, (plus 0x at the start).
*/
function leftPadHex(hexStr, n) {
  return strip0x(hexStr).padStart(n, '0');
}
...
@return {string} A binary string (padded) to size N bits.
*/
...
function hexToFieldPreserve(hexStr, packingSize, packets, silenceWarnings) {
  let bitsArr = [];
  bitsArr = splitHexToBitsN(strip0x(hexStr).toString(), packingSize);
...
function concatenateThenHash(...items) {
  const concatvalue = items
      .map(item => Buffer.from(leftPadHex(item, 64).slice(-64), 'hex'))
      .reduce((acc, item) => concatenate(acc, item));
...
function getLeafIndexFromZCount(zCount) {
  ...
  return parseInt(MERKLE_WIDTH - 1 + zCountInt, 10);
}
